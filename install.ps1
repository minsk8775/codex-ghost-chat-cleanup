...
$requiredFiles = @(
    ...
    (Join-Path $source "references\recovery.md")
)
...
New-Item -ItemType Directory -Path (Join-Path $target "references") | Out-Null
...
Copy-Item -LiteralPath (Join-Path $source "references\recovery.md") -Destination (Join-Path $target "references\recovery.md")
...
foreach ($installedFile in @(
    ...
    (Join-Path $target "references\recovery.md")
)) {
